...
def fight_sequence(hero, monster):
    print(f"{hero.name} has {hero.combat_strength} and {hero.health_points} health.")
    print(f"{monster.name} has {monster.combat_strength} and {monster.health_points} health.")

    while hero.health_points > 0 and monster.health_points > 0:
        hero_attack = hero.hero_attacks()
        monster.health_points -= hero_attack
        print(f"Hero attacks {monster.name} for {hero_attack}! Monster has {monster.health_points} health remaining")

        if monster.health_points <= 0:
            print(f"{monster.name} has been defeated!")
            hero.gain_xp(monster.xp_amount)
            print(f"{hero.name} has gained {monster.xp_amount} XP!")
            if hero.xp >= hero.xp_amount:
                print(f"{hero.name} has leveled up!")
                print(f"{hero.name} has {hero.combat_strength} and {hero.health_points} health.")
            break
        ...
        print(f"{monster.name} attacks {hero.name} for {monster_attack}!")
        print(f"{hero.name} has {hero.health_points} health remaining.")
